...
const updateProfileSchema = z.object({
  ...
  revalidation_date: z
    ...
    .optional(),
  professional_role: z
    .enum(['doctor', 'nurse', 'pharmacist', 'other', 'other_healthcare'])
    .optional(),
...
export const register = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (emailResult.success) {
    ...
    response.message =
      'Registration successful, but we could not send the verification email. Please use the resend OTP endpoint to receive your code.';
...
export const onboardingStep1 = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }
    ...
    const rolesWithKeys = roles.map((r) => ({
      ...r,
      key: deriveRoleKey(r.name),
    }));
    const availableRoleKeys = Array.from(
      new Set(rolesWithKeys.map((r) => r.key))
    );
    const availableRoles = rolesWithKeys.map((r) => ({
      name: r.name,
      status: r.status,
      type: r.type,
      key: r.key,
    ...
    await updateOnboardingStep1(req.user.userId, {
...
export const getOnboardingRoles = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }
    ...
    try {
      ...
      const parent: any[] = await prisma.$queryRawUnsafe(
        `SELECT id FROM categories WHERE LOWER(name) LIKE '%work setting%' OR LOWER(name) = 'work_settings' LIMIT 1`
      );
      if (parent && parent.length > 0 && parent[0].id) {
        const children: any[] = await prisma.$queryRawUnsafe(
          `SELECT id, name FROM categories WHERE parent_id = ${parent[0].id} ORDER BY id ASC`
        );
        if (children && children.length > 0) {
          workSettings = children.map((r: any) => ({
            id: String(r.id),
            name: r.name,
          }));
        }
      ...
      if (workSettings.length === 0) {
        ...
        const rows3: any[] = await prisma.$queryRawUnsafe(
          `SELECT id, name FROM categories WHERE name IN (${placeholders}) ORDER BY id ASC`,
          ...candidates
        );
...
export const onboardingStep2 = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }
...
export const onboardingStep3 = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }

    const validated = step3Schema.parse(
      req.body
    ) as OnboardingStep3Professional;
    ...
    res.json({
      ...
      data: {
        ...
        training_hours_completed_already:
          validated.training_hours_completed_already,
...
export const onboardingStep4 = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }
 ...
 */
export const getOnboardingDataEndpoint = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }
...
export const searchUsers = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      success: true,
      data: users,
      pagination: { total: users.length, limit, offset },
    });
  } catch (err) {
...
export const saveDiscoverySource = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }
 ...
 */
export const getDiscoverySource = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }
...
export const resetSection = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Authentication required');
    }
    ...
    switch (validated.section) {
      ...
      case 'cpd_hours':
        ...
        break;
      case 'reflections':
        await prisma.reflective_accounts.deleteMany({
          where: { user_id: userId },
        });
        await prisma.reflective_account_forms.deleteMany({
          where: { user_id: userId },
        });
        break;
      case 'feedback':
        await prisma.feedback_log.deleteMany({ where: { user_id: userId } });
        await prisma.user_feedback_logs.deleteMany({
          where: { user_id: userId },
        });
        break;
      case 'documents':
        await prisma.personal_documents.deleteMany({
          where: { user_id: userId },
        });
        break;
    }
