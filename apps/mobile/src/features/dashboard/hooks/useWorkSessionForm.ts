...
export const useWorkSessionForm = (
  ...
  const [workSettingsOptions, setWorkSettingsOptions] = useState<
    SelectionOption[]
  >([]);
  ...
  const setHoursFromTimer = useCallback((timerHours: string) => {
    if (timerHours.includes(':')) {
      ...
      if (parts.length >= 2) {
        ...
        setHours(
          `${String(outH).padStart(2, '0')}:${String(outM).padStart(2, '0')}:${String(outS).padStart(2, '0')}`
        );
      } else {
      ...
      if (Number.isFinite(val)) {
        ...
        setHours(
          `${String(outH).padStart(2, '0')}:${String(outM).padStart(2, '0')}:00`
        );
      } else {
        setHours(timerHours);
      }
  ...
  const openFormWithDefaults = useCallback(
    (timerHours: string) => {
      setHoursFromTimer(timerHours);
