...
import { ActiveSession, StartSessionDetails } from '../dashboard.types';
...
export const useActiveSession = (onSessionEnded?: () => void) => {
  ...
  const updateTimerFromSession = useCallback(
    (
      ...
      const elapsedMs = session.isPaused
        ? TimerService.calculateElapsedBetween(
            session.startTime,
    ...
    try {
      ...
      if (!token) {
        if (isMounted.current) {
          await TimerService.cancelShiftEndNotification();
          resetStore();
      ...
      if (response?.data && response.data.isActive) {
        ...
        if (storeActiveSessionId === sessionId) {
          console.log(
            '[useActiveSession] Local store is authoritative. Updating metadata only.'
          );
          // Update metadata but keep the timer state (store is authoritative)
          useTimerStore.getState().setActiveSession(session);
        } else {
          console.log(
            '[useActiveSession] Hydrating from server data (New Session/Different Session)'
          );
          setSessionData({
            id: sessionId,
  ...
  useEffect(() => {
    ...
    if (sessionIsPaused) {
      ...
      const effectivePausedAt =
        storePausedAt || activeSession.pausedAt || new Date().toISOString();
      ...
      if (storeStartTime !== activeSession.startTime)
  ...
  const handleStartSession = async (details?: StartSessionDetails) => {
    try {
      const token = await AsyncStorage.getItem('authToken');
      if (!token) {
      ...
      const durationMinutes =
        (details?.shiftHours || 0) * 60 + (details?.shiftMinutes || 0);
      const descriptionLines: string[] = [];
      if (details?.shiftType)
        descriptionLines.push(`Shift type: ${details.shiftType}`);
      if (durationMinutes > 0) {
        const h = Math.floor(durationMinutes / 60);
        const m = durationMinutes % 60;
        descriptionLines.push(`Shift length: ${h}h ${m}m`);
      }
      if (details?.location)
        descriptionLines.push(`Location: ${details.location}`);
      if (details?.notes) descriptionLines.push(`Notes: ${details.notes}`);
      const workDescription = descriptionLines.join('\n');
      ...
      const response = await apiService.post<{
        ...
        { start_time: startTime, work_description: workDescription },
        token
      );
      if (response?.data) {
        const session = response.data;
        ...
        if (durationMinutes > 0) {
          try {
            await TimerService.scheduleShiftEndNotification({
              startTime: session.startTime || startTime,
              durationMinutes,
              title: 'Shift time reached',
              body: `Your ${durationMinutes} minute shift has finished.`,
            });
          } catch (e) {
            console.warn('Failed to schedule shift notification', e);
          }
        }
  ...
  const handleResumeSession = async () => {
    ...
    const pausedAtIso =
      useTimerStore.getState().pausedAt || activeSession.pausedAt;
    if (!pausedAtIso) return;

    const resumeTime = new Date();
    const currentPauseDuration = Math.max(
      0,
      resumeTime.getTime() - new Date(pausedAtIso).getTime()
    );
    ...
    try {
      ...
      await apiService.post<{
        ...
        API_ENDPOINTS.WORK_HOURS.RESUME,
  ...
  const handleRestartSession = async () => {
    Alert.alert('Restart Session', 'Are you sure you want to start from 0?', [
        ...
        onPress: async () => {
          try {
            ...
            const response = await apiService.post<{
              ...
              API_ENDPOINTS.WORK_HOURS.RESTART,
  ...
  return {
    ...
    handleStopSession,
    setActiveSession: (s: ActiveSession | null) =>
      useTimerStore.getState().setActiveSession(s),
  };
};
